...
session_start();
include 'koneksi.php';
// munculkan / pilih sebuah atau semua kolom dari tabel user
$query = mysqli_query($koneksi, "SELECT * FROM instruktur");

// mysqli_fecth_assoc = untuk menjadikan hasil query menjadi sebuah data (object,aray)
...
if (isset($_GET['delete'])) {
    ...
    // query atau perintah untuk menghapus
                                            ...
                                            <tbody>
                                                ...
                                                while ($row = mysqli_fetch_assoc($query)) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <style>
                                                                .foto-tabel-instruktur {
                                                                    border-radius: 10px;
                                                                    width: 100px;
                                                                    border: solid 1px black;
                                                                }
                                                            </style>
                                                            <img class="foto-tabel-instruktur" width="50" src="upload/<?php echo $row['foto'] ?>" alt="">
                                                        </td>
                                                        <td><?php echo $row['nama_instruktur'] ?></td>
                                                        <td><?php echo $row['jurusan_instruktur'] ?></td>
                                                        <td>
                                                            <a onclick="" href="tambah-instruktur.php?edit=<?php echo $row['id'] ?>" class="btn btn-success btn-sm"><span class="tf-icon bx bx-pencil bx-18bx"></span>
                                                            </a>

                                                            <a onclick="return confirm('Apakah anda yakin akan menghapus data ini??')" href="instruktur.php?delete=<?php echo $row['id'] ?>" class="btn btn-danger btn-sm"><span class="tf-icon bx bx-trash bx-18bx"></span>
                                                            </a>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
